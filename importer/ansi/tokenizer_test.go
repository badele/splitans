...
func TestCSIWithSignification(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			expectedSignification: "Cursor Forward 2 times",
		},
		{
			...
			expectedSignification: "Cursor Backward 4 times",
		},
		{
			name:                  "Erase Display",
			input:                 "\x1b[2J",
...
func TestCSIWithoutParameters(t *testing.T) {
	tests := []struct {
		...
		{
			name:             "Cursor Forward without params",
			input:            "\x1b[C",
			expectedType:     TokenCSI,
			expectedNotation: "CSI Ps C",
		},
		{
			name:             "Cursor Backward without params",
			input:            "\x1b[D",
			expectedType:     TokenCSI,
			expectedNotation: "CSI Ps D",
		},
